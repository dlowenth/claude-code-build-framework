...
| Changes from v2.3 | **v2.4 - Context Efficiency and Plugin Security.** Added Section 1.7 (Context Efficiency) as a core operating principle. Updated security-framework.md to v1.1 with comprehensive plugin and skill governance: MCP/plugin auto-escalation triggers (Tier 1+ for any external connection, Tier 2+ if PII/client data). Section 12 (Plugin and MCP Security Validation) with component-level architecture — five component types (skills, tools, hooks, scripts, commands) each with distinct risk profile. Two pre-execution security gates: plugin validation checklist (12.1) for capability inputs with tool-level least privilege, and skill security content audit (12.3) for knowledge inputs with mandatory content scanning. Runtime plugin security (12.2). |
